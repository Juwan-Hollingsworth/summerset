...
export const Navbar: React.FC = () => {
  ...
  return (
    <nav className={cn(
      ...
      scrolled ? "bg-brand-paper/80 backdrop-blur-xl shadow-sm py-3 md:py-4" : "bg-transparent"
    )}>
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <Link to="/" className="flex flex-col">
          <span className="text-2xl font-serif tracking-tighter text-brand-ink leading-none">SUMMERSET</span>
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={cn(
                ...
                location.pathname === link.path ? "text-brand-olive" : "text-brand-ink/60"
              )}
            >
              {link.name}
            </Link>
        ...
        <div className="hidden md:flex items-center gap-6">
          <button className="flex items-center gap-2 text-[11px] uppercase tracking-widest font-bold text-brand-ink/40 hover:text-brand-ink transition-colors">
            <Globe size={14} />
            EN / USD
          </button>
